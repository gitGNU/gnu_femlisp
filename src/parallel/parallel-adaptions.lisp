...
(defun test-parallel-adaptions ()
  ...
  (with-workers ((lambda (k) (mp-dbg :mp "Work on ~D" k)))
    (Work-on 1)
    (work-on 2))
  (with-workers ((lambda (k)
                   (mp-dbg :mp "id=~A k=~D" *worker-id* k)
                   (with-workers ((lambda (l)
                                    (mp-dbg :mp "Working on ~D and ~D ~%" k l)))
                     (work-on 3)
                     (work-on 4))))
    (work-on 1)
    (work-on 2))
